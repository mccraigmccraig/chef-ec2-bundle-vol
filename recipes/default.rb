...
ami_bundle = node[:ami_bundle]
t = Time.now.utc
bucket = "#{ami_bundle["bucket_base"]}/#{t.strftime("%Y%m%d-%H%M")}"
image_size = ami_bundle[:image_size] || 1000
...
bash "bundle_image" do
  code <<-EOH
    ...
    $EC2_AMITOOL_HOME/bin/ec2-bundle-vol -c /tmp/aws_cert.pem -k /tmp/aws_private_key.pem -u #{aws_auth["account_number"]} -r #{aws_arch} -s #{image_size} -d /mnt
    $EC2_AMITOOL_HOME/bin/ec2-upload-bundle -m /mnt/image.manifest.xml -a #{aws_auth["access_key"]} -s #{aws_auth["secret_access_key"]} --location #{ami_bundle["location"]} -b #{bucket}
  EOH
